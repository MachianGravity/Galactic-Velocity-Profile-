...
%fclose all;
figure;
errorbar(Mea_r, Mea_v, Mea_verr,'Color',[0.23,0.67,0.20],'LineWidth',2,'Marker','o','MarkerSize',6,'MarkerFaceColor',[0.0,1.0,0.0])
hold on;
plot(Mea_r,RealV,'color',[0.87,0.33,0.00],'LineWidth',1.5,'Marker','o','MarkerSize',3,'MarkerFaceColor',[0.67,0.27,0.01]);
plot(Mea_r,YYY(:,2),':','color',[0.07,0.44,0.75],'LineWidth',1.5,'Marker','o','MarkerSize',4,'MarkerFaceColor',[0.15,0.55,0.87]);

plot(Mea_r,MONDV,'--','color',[0.93,0.69,0.13],'LineWidth',1.0,'Marker','o','MarkerSize',3,'MarkerFaceColor',[0.93,0.69,0.13]);
plot(Mea_r,NFWTotv,'--','color',[0.75,0.36,0.98],'LineWidth',1.0,'Marker','o','MarkerSize',3,'MarkerFaceColor',[0.75,0.38,0.98]);


%h = legend({'Observed Velocity','Machian Gravity','Newtonian Velocity','MOND','NFW Profile'}, ...
%       'Location','northwest', ...
%       'FontSize', 25);


% ---- Axis and labels ----
ylabel('V (km/s)', 'FontSize', 30);
...
h = text(0.98, 0.02, galname, ...
     ...
     'FontSize', 25,'EdgeColor','k','BackgroundColor','w');

pos = h.Position;       % [x y width height]

% Shift legend slightly to the right and down
pos(1) = pos(1) - 0.02;   % move right (increase x)
...
filename = ['./Fig6plot/Galaxy_Velocity_Profile/' num2str(i)];
